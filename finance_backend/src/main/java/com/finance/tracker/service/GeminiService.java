...
public class GeminiService {
    ...
    public List<Transaction> processChat(String userInput, List<Transaction> history) {
        ...
        try {
            ...
            for (JsonNode data : dataArray) {
                ...
                if (transaction.getAmount().compareTo(BigDecimal.ZERO) == 0 && !type.equals("CHAT")) continue;

                transaction.setType(type);
                
                LocalDate txDate = LocalDate.now();
                if (data.has("date") && !data.get("date").isNull()) {
                    try { txDate = LocalDate.parse(data.get("date").asText().trim()); } catch (Exception ex) {}
                }
            ...
            if (statusCode == 429) errorTx.setNote("ERROR|AI đang nghỉ giải lao (Hết hạn mức). Bạn đợi 1 phút nhé!");
            else if (statusCode == 503) errorTx.setNote("ERROR|Máy chủ Google đang quá tải. Bạn bấm gửi lại lần nữa nhé!");
            else errorTx.setNote("ERROR|Lỗi kết nối API: " + statusCode);
            return List.of(errorTx);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public Transaction processReceiptImage(MultipartFile file) {
        try {
            ...
            transaction.setIsAnomaly(data.path("isAnomaly").asBoolean(false));
            transaction.setNote(data.path("categoryName").asText("Khác") + "|" + data.path("note").asText("Hóa đơn ảnh"));
